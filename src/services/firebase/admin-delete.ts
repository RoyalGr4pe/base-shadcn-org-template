...
// Local Imports
import { inviteCodesCol, organisationsCol, usersCol } from "@/constants/collections";
...
async function deleteUserDocsAdmin({ uid, idToken, isOwner, orgId }: { uid: string, idToken: string, isOwner?: boolean, orgId?: string }): Promise<{ success?: boolean; error?: unknown }> {
    try {
        ...
        if (isOwner && orgId) {
            ...
            // Step 5: Delete all invite codes for the organisation
            const inviteCodesQuery = await firestoreAdmin
                .collection(inviteCodesCol)
                .where('orgId', '==', orgId)
                .get();

            const inviteBatch = firestore().bulkWriter();
            inviteCodesQuery.docs.forEach((docSnap) => {
                inviteBatch.delete(docSnap.ref);
            });
            await inviteBatch.close();
        }
...
export async function deleteInviteCodeAdmin({ idToken, inviteId, orgId }: { idToken: string, inviteId: string, orgId: string }): Promise<{ success?: boolean; error?: string }> {
    try {
        ...
        // Step 4: Delete the invite code
        await firestoreAdmin.collection(inviteCodesCol).doc(inviteId).delete();
